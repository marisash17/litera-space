...
class RakPinjamController extends Controller
    {
        ...
        public function pinjam($id)
        {
            ...
            // Pastikan user punya member
            $member = auth()->user()->member;
            if (!$member) {
                return redirect()->back()->with('error', 'Akun Anda belum terhubung dengan member. Silakan hubungi admin.');
            }
            ...
            Peminjaman::create([
                'buku_id' => $book->id,
                'member_id' => $member->id, // pastikan selalu tidak null
                'user_id' => auth()->id(),
        ...
        public function kembalikan($id)
        {
            ...
            // Update peminjaman terakhir user
            $peminjaman = Peminjaman::where('buku_id', $book->id)
                                    ->where('member_id', auth()->user()->member->id ?? 0)
